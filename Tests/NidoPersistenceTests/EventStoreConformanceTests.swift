...
class EventStoreConformanceTests: XCTestCase {
    ...
    func testAppendedEventsCanBeReadBack() async throws {
        let recorded = try await store.apply(RecordWakeCommand(householdID: household, occurredAt: day.at(7, 4), source: .app), now: day.at(7, 4))
        let stored = try await store.allEvents()
        let identifier = try XCTUnwrap(recorded.first).id
        let found = try await store.event(id: identifier)

        XCTAssertEqual(stored.count, 1)
        XCTAssertEqual(stored.first?.id, identifier)
        XCTAssertEqual(found?.type, .childWoke)
    }

    func testAppendingTheSameEventTwiceStoresItOnce() async throws {
        let recorded = try await store.apply(RecordWakeCommand(householdID: household, occurredAt: day.at(7, 4), source: .app), now: day.at(7, 4))
        try await store.append(recorded)
        try await store.append(recorded)
        let stored = try await store.allEvents()
    ...
    func testARetriedCommandDoesNotRecordTwice() async throws {
        ...
        XCTAssertEqual(stored.count, 1, "the same tap delivered twice is one event")
    }
    ...
    func testLedgerSeesTheCurrentRevisionOnly() async throws {
        ...
        let ended = try await store.apply(EndSleepCommand(householdID: household, occurredAt: day.at(11, 31), source: .watch, ruleID: nap, sleepType: .nap1), now: day.at(11, 31))
        let end = try XCTUnwrap(ended.first)
        ...
        XCTAssertEqual(stored.count, 3, "history is kept")
    ...
    func testDeletingLeavesATombstoneRatherThanAHole() async throws {
        let recorded = try await store.apply(LogWaterCommand(householdID: household, occurredAt: day.at(9, 0), source: .app), now: day.at(9, 0))
        let logged = try XCTUnwrap(recorded.first)
        ...
        XCTAssertEqual(stored.count, 2, "the tombstone is a revision, not an erasure")
        XCTAssertTrue(ledger.effectiveEvents.isEmpty, "and nothing stands afterwards")
    }
    ...
    func testLocallyRecordedEventsAreQueuedForUpload() async throws {
        ...
        XCTAssertEqual(pending.map(\.id), recorded.map(\.id))
    }

    func testUploadedEventsLeaveTheOutbox() async throws {
        let recorded = try await store.apply(RecordWakeCommand(householdID: household, occurredAt: day.at(7, 4), source: .app), now: day.at(7, 4))
        try await store.markUploaded(recorded.map(\.id))

        let pending = try await store.pendingUploads()
        let stored = try await store.allEvents()

        XCTAssertTrue(pending.isEmpty)
        XCTAssertEqual(stored.count, 1, "leaving the outbox does not leave the ledger")
    }
    ...
    func testEventsArrivingFromElsewhereAreNotQueuedForUpload() async throws {
        ...
        XCTAssertEqual(stored.count, remote.count)
        XCTAssertTrue(pending.isEmpty, "the cloud already has what the cloud sent us")
    }
    ...
    func testMergingKeepsBothSidesOfADisconnection() async throws {
        ...
        XCTAssertEqual(stored.count, remote.count + 1, "an offline device is not wrong")
    }
